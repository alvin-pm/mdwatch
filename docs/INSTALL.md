...
- 미설치 시 **D2Coding 웹폰트**(jsDelivr CDN, 1.4MB 최초 1회 캐시)로 자동 폴백됩니다 — 설치 여부와 무관하게 문자 폭이 D2Coding 기준으로 동일해서 박스문자(`┌─│`)·화살표(`→↑`)까지 정렬이 유지됩니다. 그래도 로컬 설치를 권장하는 이유: 최초 로드 지연 없음 + 오프라인 동작 (오프라인 + 미설치면 일반 고정폭 폰트 폴백 → 정렬 어긋날 수 있음).
