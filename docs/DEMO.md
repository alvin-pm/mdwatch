...
언어가 없는 블록은 평문 그대로 유지됩니다 — 텍스트 다이어그램이 오염되지 않습니다. (깨지지 않는 다이어그램 작성 규칙은 [DIAGRAM-GUIDE.md](DIAGRAM-GUIDE.md) 참조)
...
박스 문자·화살표도 D2Coding 폭 기준으로 정렬됩니다 (폰트 스택이 D2Coding 웹폰트까지 통일되어 있어 미설치 환경에서도 동일):
